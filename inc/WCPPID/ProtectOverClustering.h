...
namespace WCPPID{
void Protect_Over_Clustering(std::vector<std::pair<int, WCP::Opflash*> >& to_be_checked,
			     ...
			     WCP::ToyCTPointCloud& ct_point_cloud, bool flag_match_data, int run_no, double time_offset, double nrebin, double time_slice_width
			       );
  int get_next_cluster_id(int acc_cluster_id, std::set<int>& used_cluster_ids);

  std::pair<double, double> compare_pe_pattern(int run_no, double offset_x, WCP::Photon_Library *pl, WCP::SMGCSelection& mcells, WCP::Opflash* flash, bool flag_match_data);
